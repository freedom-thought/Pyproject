...
def user_recog(data_file_path_root2, result_file_path_root, model_file_path_root):
    ...
    for i in labels:
        ...
        # 基于随机森林算法的特征选择
        x_select = []
        data_full4 = data_full3.drop(["cluster_label"])
        ...
        features_importance = model3.feature_importances_
        indices = np.argsort(features_importance)[::-1]
        for m in range(data_full4.iloc[:, :15].shape[1]):
            if m <= 10:
                x_select.append(data_full4.iloc[:, :15].columns[indices[m]])

        data_full5 = pd.DataFrame(data_full4, columns=x_select)
        data_train, data_test = train_test_split(data_full5.values, test_size=0.3,
                                                 random_state=np.random.RandomState(),
                                                 shuffle=True)
        x_train_, x_test_ = data_train[:, :15], data_test[:, :15]

        # 分类预测--基于逻辑回归算法
        model3 = LogisticRegression(C=10, l1_ratio=0.03, random_state=np.random.RandomState(), tol=0.01)
        ...
        data11 = pd.DataFrame(data10, columns=x_select)

        neg_label_pred = model3.predict(data11).reshape(-1, ).tolist()
        neg_label_pred_prob = np.mean(model3.predict_proba(data11), axis=1).reshape(-1, ).tolist()
...
def terminal_info(result_file_path_root, sql1, sql2, sql3):
    a = pd.read_csv(filepath_or_buffer=result_file_path_root + "5g_phone_user_pred4_2020-03-18.csv", encoding="utf-8",
                    error_bad_lines=False,
                    skip_blank_lines=True,
                    low_memory=True, engine="c")

    a.columns = ["user_id", "phone_no", "if_use_long", "if_flow_high", "if_value_high",
                 "if_phone_price_high", "if_change_short", "if_deadline", "pred_prob"]
...
def result_concat(data1, data2, result_file_path_root):
    data1.columns = ["user_id", "phone_no", "if_use_long", "if_flow_high", "if_value_high",
                     "if_phone_price_high", "if_change_short", "if_deadline", "pred_prob"]
    data2.columns = ["user_id", "phone_brand1", "phone_brand2", "brand_price_interval"]

    date_now = datetime.datetime.now()
    final_result = pd.concat([data1, data2], axis=1, sort=False, ignore_index=True,
                             keys=["user_id"]).reset_index(drop=True)
    final_result.to_csv(result_file_path_root + "5g_phone_user_pred_all_" + str(date_now) + ".csv",
                        encoding="utf-8", mode="w",
                        header=["user_id", "phone_no", "label_pred", "label_prob_pred", "if_use_long", "if_flow_high",
                                "if_value_high",
                                "if_phone_price_high", "if_change_short",
                                "if_deadline", "phone_brand1", "phone_brand2",
                                "brand_price_interval"], index=False)
    print("模型的预测结果已写入文件！")
...
if __name__ == '__main__':
    ...
    data_file_path_ = "/kafka/model/hb_model_test/model_2020/5g_phone_user_pred/data/wkm/"
    ...
    hive_data_read(sql1=sql11_, sql2=sql12_, sql3=sql13_, data_file_path_root1=data_file_path_)
    print("训练数据已生成！")
    start_time1 = datetime.datetime.now()
    data_full2, result2 = user_recog(data_file_path_root2=data_file_path_, model_file_path_root=model_file_path_root_,
    ...
    data1_, data2_ = terminal_info(result_file_path_root=result_file_path_root_, sql1=sql31_, sql2=sql32_, sql3=sql33_)
    ...
    result_concat(data1=data1_, data2=data2_, result_file_path_root=result_file_path_root_)
    end_time3 = datetime.datetime.now()
    print("终端偏好识别模型和价格挡位偏好分析模型训练和预测任务已完成！")
    time_interval3 = format(end_time3 - start_time3)
    print("此次训练和预测共耗时：", time_interval3)
